...
{	
  ...
  vec3 normalSCO_norm = normalize(normalSCO); //Normalitzem les normals
  vec3 L = normalize(posFocus - vertexSCO.xyz);

  FragColor = vec4(Ambient() + Difus(normalSCO_norm,L,colFocus) + Especular(normalSCO_norm,L,vertexSCO,colFocus),1.0);
