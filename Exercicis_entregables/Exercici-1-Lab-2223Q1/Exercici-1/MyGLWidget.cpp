...
{
  glm::mat4 TG(1.0f);
  TG = glm::translate(TG,glm::vec3(-0.2,0.15,0.0)); //Translació desde el (0,0,0) fins a la posició (-0.2,0.15,0.0)

  if (esc == 2) {  //Si estem a l'escena de la celebració caldrà rotar sobre les espatlles
    TG = glm ::translate(TG,glm::vec3(0.05,0.0,0.0)); //Retornem a la posició inicial
    TG = glm::rotate(TG,float(-120*M_PI/180.0),glm::vec3(0.0, 0.0, 1.0)); //Rotem sobre l'eix z (cap a l'esquerra)
    TG = glm ::translate(TG,glm::vec3(-0.05,0.0,0.0)); //Col·loquem centre espatlla al (0,0,0)
  }

  TG = glm::scale(TG,glm::vec3(0.2,0.2,1.0));  //Escalem per tal que la llargada sigui 0.4 i l'amplada 0.1
  TG = glm::translate(TG,glm::vec3(0.25,-1.0,0.0));  //Posem el punt de dalt a l'esquerra al (0,0,0) per controlar on queda el braç després d'escalar
  glUniformMatrix4fv(TGLoc, 1, GL_FALSE, &TG[0][0]); 
}
...
void MyGLWidget::modelTransformBracEsquerre()
{
  ...
  TG = glm::translate(TG,glm::vec3(0.1,0.15,0.0)); //Translació desde el (0,0,0) fins a la posició (0.1,0.15,0.0) (En la que ha d'estar el vèrtex superior esquerra del braç)

  if (esc == 2) {  //Si estem a l'escena de la celebració caldrà rotar sobre les espatlles
    TG = glm ::translate(TG,glm::vec3(0.05,0.0,0.0)); //Retornem a la posició inicial
    TG = glm::rotate(TG,float(120*M_PI/180.0),glm::vec3(0.0, 0.0, 1.0)); //Rotem sobre l'eix z cap a la dreta
    TG = glm ::translate(TG,glm::vec3(-0.05,0.0,0.0)); //Col·loquem centre espatlla al (0,0,0)
  }
...
{
  glm::mat4 TG(1.0f);
  TG = glm::translate(TG,glm::vec3(-0.15,0.5,0.0)); //Translació per colocar la corona a sobre el cap
  TG = glm::scale(TG,glm::vec3(0.2,0.2,1.0)); //Escalem per a llargada = 0.4 i amplada = 0.3
  TG = glm::translate(TG,glm::vec3(0.75,1.0,0.0));  //Posem el punt de baix a l'esquerra al (0,0,0) per controlar on queda la corona després d'escalar
  glUniformMatrix4fv(TGLoc, 1, GL_FALSE, &TG[0][0]); 
}
...
void MyGLWidget::keyPressEvent(QKeyEvent* event)
{
  ...
  switch (event->key()) {
    case Qt::Key_C :
      esc += 1; //Avancem d'escena
      if (esc == 3) esc = 0; //Tornem a l'escena inicial
      break;
