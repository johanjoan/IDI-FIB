...
void MyGLWidget::creaBuffers() {
    ...
    m1.load ("models/legoman.obj"); //Carreguem model Patricio
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat)*m1.faces().size()*3*3, m1.VBO_vertices(), GL_STATIC_DRAW); //Posició
    glVertexAttribPointer(vertexLoc, 3, GL_FLOAT, GL_FALSE, 0, 0);
    glEnableVertexAttribArray(vertexLoc);
...
void MyGLWidget::paintGL() {
    ...
    if (model_pintar) glBindVertexArray (VAO1);
...
void MyGLWidget::modelTransformLego() {
    ...
    transform = glm::translate(transform,posicio_patricio); //El movem a la posició que toqui (Cada patricio a una posició diferent)
    transform = glm::rotate(transform,rotation,glm::vec3(0.0,1.0,0.0)); //Rotem el Patricio
    transform = glm::scale(transform,glm::vec3(escala_lego)); //Escalem per a tindre alçada 1
    transform = glm::translate(transform,-base_lego); //Col·loquem el patricio al (0,0,0)
    glUniformMatrix4fv(transLoc, 1, GL_FALSE, &transform[0][0]);
}
...
void MyGLWidget::calcula_escena() { //Calcula els punts maxims i minims de l'escena i la base del patricio
    ...
    for (unsigned int i = 3; i < m1.vertices().size(); i+=3) {
        ...
        if (m1.vertices()[i+1] > maxy) maxy = m1.vertices()[i+1];
        if (m1.vertices()[i+2] < minz) minz = m1.vertices()[i+2];
        if (m1.vertices()[i+2] > maxz) maxz = m1.vertices()[i+2];
    }
    base_lego = glm::vec3((minx+maxx)/2.0,miny,(minz+maxz)/2.0); 
    escala_lego = 1.0/(maxy-miny); //Volem altura 1
...
void MyGLWidget:: mouseMoveEvent(QMouseEvent *event) {
    ...
    if (event->type()== QEvent::MouseMove and event->buttons() == Qt::LeftButton) { //Movem només si està el botó esquerra premut
        ...
        //Posem el límit dels angles entre 0 i 2*PI
        if (theta > 2*M_PI) theta -= 2*M_PI;
        else if (theta < 0) theta += 2*M_PI;
        if (phi > 2*M_PI) phi -= 2*M_PI;
        else if (phi < 0) phi += 2*M_PI;
        emit envia_phi(phi*180/M_PI);
        emit envia_theta(theta*180/M_PI);
        x_old = x_new;
        y_old = y_new;
        viewTransform();
        update();
...
void MyGLWidget::set_model(bool press) {
    makeCurrent();
    model_pintar = !press;
    update();
}

void MyGLWidget::set_phi(int angle) {
    makeCurrent();
...
void MyGLWidget::set_theta(int angle) {
    makeCurrent();
...
void MyGLWidget::set_color_terra(int r, int g, int b) { //Realment canvia el color de fons
    makeCurrent();
    //Creem el terra
